...
export default function work() {
    ...
    return (
        <motion.div style={{ scale }} className="w-[100dvw] bg-white rounded-t-lgxl relative flex flex-col justify-self-center lgxl:justify-items-between justify-items-center justify-center gap-5 ">
            <div className="sm:max-w-[90vw] max-w-[85vw] self-center">
                <motion.div style={{ opacity }}>
                    <h1 className="lg:text-[36px] lgxl:text-[50px] font-bold mt-[60px]">O que me torna diferente</h1>
                    ...
                    {CARDS.map((item, index) => {
                        ...
                        return (
                            <motion.div key={index} ref={ref} initial={{ opacity: 0, y: 100 }} animate={isInView ? { opacity: 1, y: 0 } : {}} whileHover={{ scale: 1.05 }} transition={{ duration: 0.5, delay: index * 0.2 }}
                                className={`flex-col justify-center items-center lg:w-[380px] lg:h-[410px] lgxl:h-[600px] lgxl:w-[550px] gap-1 bg-[#D7DDD1] rounded-2xl shadow-lg`}>
                                <img className="w-full lgxl:h-[350px] lgxl:w-[550px]" src={item.image} />
                                <p className="text-center lgxl:text-[28px] lg:text-[18px] lgxl:mx-12 lgxl:mt-6 font-bold">{item.title}</p>
                                <p className="text-center lg:text-[16px] lgxl:text-[22px] lgxl:mx-12 lgxl:mt-3">{item.content}</p>
                            </motion.div>
                        );
                    })}
                </div >
                ...
                <div className="w-full justify-items-center">
                    <p className="lg:text-[16px] lgxl:text-[22px] my-[25px]">Talvez você só precise de um lugar onde possa ser ouvida(o) — de verdade. E esse lugar <a href="/" className="underline">começa aqui</a>.</p>
